...
RSpec.describe MasterAdmin::Trailer::Camera::Enable do
  describe '#call' do
    subject do
    ...
    context 'when camera exists' do
      ...
      let!(:trailer_camera) { create(:trailer_camera, installed_at: nil, trailer_id: trailer.id) }
      let(:params) { trailer_camera.id }

      it 'changes the installed at timestamp' do
        subject
        expect(trailer_camera.reload.installed_at).not_to be_nil
      end
    end
  end
end
