...
RSpec.describe TrailerDataUsage::ParseReport do
  ...
  describe '#call' do
    context 'with valid report' do
      before do
        allow_any_instance_of(described_class).to receive(:current_report?).and_return(true)
      end
    ...
    context 'without report' do
      before do
        allow_any_instance_of(described_class).to receive(:fetch_report).and_return(nil)
      end
    ...
    context 'with already_sync report' do
      before do
        allow_any_instance_of(described_class).to receive(:already_sync?).and_return(true)
      end
